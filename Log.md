...
## 19. 출시 준비 — Play Console 등록 & iOS 이전 준비 (2026-06-03)
...
### 19-2. Google Play Console 앱 등록 진행
...
- 필요 절차:
  ...
  2. ~~새 Google Play 계정 등록 ($25 추가) — 기존 개인 계정은 Organization으로 변환 불가~~
     → **[2026-08-29 정정] 사실이 아님. 제자리 전환이 공식 지원된다.** 23장 참조.
...
### 19-7. 다음 세션에서 이어갈 작업
- [x] D-U-N-S 번호 신청 (무료, 한국기업데이터 사이트) — 2026-08-29 완료
- [x] 현재 앱 출시 전략 최종 결정 → 옵션 1·2 모두 폐기, **제자리 전환** 채택 (23장)
...
- ~~**개인 계정 → Organization 계정 직접 변환 불가**: 새 계정 별도 등록 후 앱 이전 신청 필요.~~
  → **[2026-08-29 정정] 오류.** Google은 개인→조직 **제자리 전환**을 공식 지원한다
  (`개발자 계정 → 내 정보 → 계정 유형 변경`). 새 계정 등록($25 재결제)도 앱 이전도 불필요하며
  개발자 계정 ID·앱·패키지명·서명키가 전부 그대로 승계된다. 실제 전환 완료 기록은 23장.
...
## 21. App Store 심사 1차 반려(2.1 정보요청) 및 회신 (2026-08-19)

### 21-1. 반려 내용

제출 다음 날 새벽 `Guideline 2.1 - Information Needed` 로 반려. **기능 결함이 아니라 정보 요청** —
실기기 화면 녹화 + 7개 항목(기능·대상, 테스트 기기, 사용법, 외부 서비스, 지역 차이, 규제/제3자 자료)을
요구하는 최근 Apple 의 신규 제출 정형 템플릿 (동일 문구 반려 사례 다수 확인).
빌드 재업로드 불필요, 회신만으로 심사 재개되는 유형.

### 21-2. 실기기 영상 촬영 — 음성 가이드 방식

Apple 이 "physical device 에서 촬영" 을 명시해 시뮬레이터/웹 합성 불가. 실물 아이폰 터치는 자동화가
불가능하므로 역할 분담: **Claude 가 앱 삭제→재설치(권한 팝업 리셋) + `say -v Yuna` 한국어 음성으로
단계별 가이드(맥 스피커) + 검수·인코딩, 사용자는 안내를 따라 터치만.**

- 가이드 대본은 `[[slnc ms]]` 무음 마커로 단계 간 대기 시간을 박아 한 번의 `say` 호출로 진행
- 아이폰 자체 화면 기록(제어 센터) 사용 — QuickTime 은 "김학성의 iPhone" 소스 선택은 되는데
  화면이 안 비치는 문제로 포기
- 1차 시도는 완전 암송에서 사용자가 구절 준비가 안 돼 중단 → 앱 재설치 후 재촬영.
  개선: 완전 암송 진입 전 "난이도 선택 화면 하단 회색 상자의 구절 전문을 눈으로 익히는" 단계 추가
- 결과물: 6분 34초, 1180×2556 60fps, 497MB

### 21-3. 프레임 검수 (ffmpeg 컨택트 시트)

15초 간격 27프레임 시트로 전체 흐름 확인 후, 권한 팝업 구간은 1~2초 간격 정밀 스캔.

- ✅ 앱 실행(스플래시)부터 시작 / 메인 3부서 / 부서 화면 / 쉬움 모드 완료 토스트
- ✅ **음성인식+마이크 권한 팝업, 허용 탭 장면** / 실시간 음성→텍스트 변환
- ✅ 검색("사랑") / 캘린더 / 설정 알림 토글 / 위젯 추가 + 부서 변경(유치부→중고등부)
- ⚠️ 알림 권한 팝업 없음 — 이 앱은 **알림 스케줄을 추가하는 시점**에 OS 권한을 요청하는 구조라
  토글만 켠 흐름에서는 안 뜸. 회신문에 사실대로 기재하고 재촬영하지 않기로 판단.

인코딩: `-an -vf scale=886:1920,fps=30 -crf 24` → **17MB**. `review-assets/`(gitignore) 보관.

### 21-4. 회신 제출

- 영문 회신문(7개 항목)은 `AppReviewReply.md`. ASC 회신창 4,000자 제한에 걸려 3,368자로 압축.
- 7항에 성경 판본 명시: **개역한글(1961), 대한성서공회 단체명의 저작물로 구 저작권법상
  보호기간(공표 후 50년) 만료** — 처음 개역개정으로 잘못 알았다가 정정.
- 첨부가 Chrome 에서 조용히 실패하는 것처럼 보여 **GitHub 릴리즈 자산으로도 업로드**
  (`releases/download/review-demo-v1/church-memory-review-demo.mp4`) 후 본문에 링크 추가.
  실제로는 첨부도 올라가 있었음 → 최종적으로 첨부 2개 + 링크 삼중 전달.
- **2026-08-19 19:05 회신 전송 완료. Apple 응답 대기 중.**

### 21-5. 트러블슈팅 메모

- **ASC 회신창은 4,000자 제한** — 회신문을 길게 쓰면 붙여넣기 자체가 거부됨. 사전 압축 필요.
- **ASC 파일 첨부는 썸네일/미리보기가 없음** — 파일명 표시만으로 판단해야 하며, Chrome 에서는
  그마저 안 보일 수 있음. 공개 다운로드 링크를 본문에 병기하면 확실.
- **QuickTime iPhone 화면 미러링 불안정** — 소스 선택은 되나 화면이 안 나오는 경우가 있음.
  아이폰 자체 화면 기록이 더 확실하고 화질도 좋음 (1180×2556 60fps).
- **`say` 의 `[[slnc ms]]` 마커**로 무음 구간을 넣으면 sleep 없이 단계별 음성 가이드를 한 번에 재생 가능.
- **재촬영 대비**: 권한 팝업은 한 번 허용하면 다시 안 뜨므로, 재촬영 전 앱 삭제→재설치 필수.
- 검수는 ffmpeg `tile` 필터 컨택트 시트(15초 간격 → 의심 구간 1초 간격)가 프레임 낭비 없이 효율적.

---

**문서 갱신**: 2026-08-19
**작성**: Claude Code 세션 (심사 반려 대응 — 실기기 영상 촬영·검수·회신 라운드)

---

## 22. App Store 1.0 정식 출시 🎉 (2026-08-22)

- 8/19 심사 회신(실기기 영상 + 7개 항목) → **8/22 승인 및 자동 출시**. 추가 질문 없이 한 번에 통과.
- 스토어 URL: https://apps.apple.com/kr/app/id6802671002
- 출시 시각: 2026-08-22 21:24 KST (iTunes Lookup API 확인)
- 구성: 1.0 (1) / iPhone 전용 / iOS 15.0+ (위젯 17.0+) / 대한민국 / 무료 / 4+

### 남은 정리

- [ ] GitHub 릴리즈 `review-demo-v1` (심사용 영상) — 역할 끝났으므로 삭제 가능
- [ ] Play Store 업데이트 — `HANDOFF-WINDOWS.md` 로 윈도우에서 진행 (무한 리렌더·헤더 겹침·음성인식 반영) → 23장에서 착수
- [ ] 키스토어 백업 확인

---

**문서 갱신**: 2026-08-23
**작성**: Claude Code 세션 (App Store 출시 확인)

---

## 23. Play Console 조직(사업자) 계정 제자리 전환 (2026-08-29)

### 23-1. 19장 결론 정정 — 제자리 전환이 공식 지원된다
19-6 / 19-8에 "개인 계정은 Organization으로 변환 불가, 새 계정 등록 후 앱 이전 필요"라고
적었으나 **오류**였다. Google 공식 문서 기준 개인→조직 **제자리 전환**이 지원된다.

- 근거: [Google 결제 프로필로 관리되는 개발자 ID 세부정보 업데이트](https://support.google.com/googleplay/android-developer/answer/16260648?hl=ko)
- 반대 방향(조직 → 개인)만 불가. 이 경우에만 새 계정 + 앱 이전이 필요하다.
- 결과적으로 **$25 재결제 불필요, 앱 이전(1~2주) 불필요**, 기존 진행 상황 100% 승계.

### 23-2. 전환 절차 (실제 수행 순서)
1. 조직 공식 **웹사이트 인증** — `https://testbench.tools/` 등록 후 인증 요청.
   이게 끝나야 `계정 유형 변경` 버튼이 노출된다.
2. `개발자 계정 → 내 정보 → 계정 유형 변경` → 조직 선택
3. 조직용 **새 결제 프로필 생성** — 기존 개인 결제 프로필은 유형 변경이 불가하므로 반드시 신규 생성
4. **D-U-N-S 번호** 입력 + 조직 유형/규모/전화번호 입력
5. Google 연락용 · 공개 표시용 이메일/전화 **OTP 인증**
6. 법인명·주소 서류 심사 통과
7. 내 정보로 복귀 → **"결제 프로필을 개발자 계정에 연결"** → **"확인 및 저장"**
   ← 이 마지막 단계가 실제 전환 트리거. 결제 프로필 인증만으로는 전환되지 않는다.

### 23-3. 전환 결과 (2026-08-29 16:50 완료 메일 수신)
| 항목 | 값 |
|------|-----|
| 계정 유형 | 개인 → **조직** |
| 개발자 계정 ID | `7551620893979929576` (**변경 없음**) |
| 개발자 이름 | Haku. → **TECHSENSE** |
| 조직명 | 테크센세 |
| 계정 소유자 | makseong@gmail.com (김학성) |
| 인증 웹사이트 | https://testbench.tools/ ✅ |
| 연락처·개발자 이메일 | contact@testbench.tools ✅ |
| 앱 / 패키지명 / 서명키 | `com.church.memory.app` 그대로 승계 |

### 23-4. 12명/14일 요구사항 — 앱을 새로 만들 필요는 없다
계정 유형은 **계정 단위 속성**이고 앱은 거기에 소속될 뿐이다. 따라서 계정이 조직으로 전환되면
기존 앱도 조직 계정의 앱이 된다. 12명/14일은 "개인 계정에 속한 앱"에 걸리는 게이트지
앱 생성 시점에 고정되는 속성이 아니다.

앱을 새로 등록하면 오히려 손해다:
- 패키지명 `com.church.memory.app` 은 이미 이 계정에 등록되어 재사용 불가 → 다른 이름을 써야 함
- 그러면 이미 App Store 에 나간 iOS 빌드와 패키지명이 어긋남
- Android 개발자 인증의 서명키 매핑, 기존 스토어 등록정보 작업이 모두 무효화

단, 공식 문서에 "전환하면 면제된다"는 **명시적 문장은 없다**. "조직 계정은 면제 대상"이고
판정 기준이 계정 유형이라는 데서 나오는 결론이다. 실제 해제 여부는 대시보드 `프로덕션` 카드의
`프로덕션 신청` 버튼 활성화로 확인할 것. 안 풀리면 계정 ID와 함께 지원팀 문의.

- 공식 권고: **전환 완료 후 최소 72시간 경과 뒤 앱 제출** (→ 2026-09-01 오후 이후)

### 23-5. 릴리즈 서명을 Gradle 로 이관
기존에는 Android Studio 마법사에만 의존해 CLI 재현이 불가능했다.
`keystore.properties` 기반으로 옮겨 `gradlew bundleRelease` 로도 동일 서명이 나오게 했다.
(HANDOFF-WINDOWS.md 의 마법사 절차도 그대로 유효하다.)

- `android/app/build.gradle`: `signingConfigs.release` 추가. `keystore.properties` 가 없으면
  서명 없이 빌드되도록 명시 분기 — debug 키로 서명된 AAB 가 나가는 사고를 방지
- `android/keystore.properties.example` 신규 (템플릿)
- 파일명은 루트 `.gitignore` 38–42행에 이미 등록된 `keystore.properties` 규약을 따랐다.
  서명 파일 무시 규칙은 이미 갖춰져 있어 추가 변경하지 않음
- ⚠️ WSL 에 Java 가 없어 **빌드 검증 미실시**. 윈도우에서 확인 필요

### 23-6. 기존 릴리즈 서명 키 지문 (분실 대비)
`android/app/release/app-release.aab` (2026-06-03 빌드)에서 추출.

```
subject : CN=Hakseong Kim, OU=Dev, O=Church Memory App, L=Korea
별칭    : KEY0
만료    : 2051-05-28
SHA-256 : 84:CA:B2:43:5A:1F:9E:4E:29:7C:3A:28:FA:6C:FC:E2:
...
- 10장 "앱 운영자 정보" 신설 (테크센세/TECHSENSE, 대표 김학성, testbench.tools)
- 11장 문의처: `makseong@gmail.com` → **`contact@testbench.tools`**
- 주소는 자택 겸용이라 본문에 미기재 (조직 계정은 Play 개발자 프로필에 주소가 공개 표시됨)

### 23-8. 저장소 운영 메모 — 윈도우/맥 충돌 방지
이번 세션 초기에 윈도우 워킹카피가 **14커밋(약 3개월) 뒤처진 상태**로 작업을 시작해
`Log.md` 장 번호가 어긋나는 문제가 생겼다. 원인은 저장소 구조가 아니라 pull 누락이었다.

- **저장소 분리는 하지 말 것.** Capacitor 구조상 `client/` 웹소스가 iOS·안드로이드 공통
  알맹이다. 실제로 20~22장의 "iOS 커밋" 14개 중 `client/` 변경이 24개 파일 +764/-464 인 반면
  `android/` 변경은 런처 아이콘 PNG 10개뿐이었다. 안드로이드가 받아가야 할 무한 리렌더·
  헤더 겹침·음성인식 수정이 전부 그 "iOS 커밋" 안에 들어있다. 쪼개면 이 흐름이 끊긴다
- **작업 시작 전 `git pull` 필수.** 이번 건은 이것만으로 발생하지 않았다
- **플랫폼별 브랜치 + PR.** 맥은 `main`, 윈도우는 작업 브랜치를 파서 합친다
...
- [x] AAB 업로드 — 내부 테스트를 건너뛰고 **프로덕션 직행** (24장). 업로드 키 정상 확정됨
...
- [ ] 스토어 등록정보: 그래픽 1024×500, 스크린샷 2장 이상, 콘텐츠 등급 설문, 데이터 보안 양식
- [ ] 스토어 등록정보의 개발자 표시명이 TECHSENSE 로 바뀐 점 반영
- [ ] `HANDOFF-WINDOWS.md` 에 조직 계정 전환 및 서명 Gradle 이관 반영
- [ ] Apple Developer 도 Organization 으로 등록 검토 (동일 D-U-N-S 재사용 가능)

---

**문서 갱신**: 2026-08-29
**작성**: Claude Code 세션 (조직 계정 제자리 전환 + 릴리즈 서명 Gradle 이관 라운드)

---

## 24. Google Play 프로덕션 제출 (2026-08-29)

### 24-1. 12명/14일 면제 — 화면으로 확인됨
23-4 에서 "공식 문서에 명시적 문장이 없어 장담할 수 없다" 고 적었으나, 조직 전환 후
**대시보드에서 게이트가 실제로 사라진 것을 확인**했다.

전환 전 (개인 계정):
```
프로덕션 액세스 신청
  ○ 비공개 테스트 버전 게시
  ○ 0명 이상의 테스터가 비공개 테스트 참여를 선택   ← 현재 0명
  ○ 0명 이상의 테스터를 대상으로 0일 이상 실행
  [ 프로덕션 신청 ]  ← 비활성
```

전환 후: 위 카드가 통째로 사라지고 **`버전 생성 및 게시`** (프로덕션 트랙 직행) 로 대체됨.
비공개 테스트는 선택사항으로 내려갔다. **결론: 조직 계정 전환으로 요구사항이 면제된다.**
...
| 국가/지역 | 대한민국 (iOS 와 동일) |
| 신규 설치 크기 | 4.47MB / 다운로드 2초 |
| 타겟 SDK | 35 (신규 앱 요건 충족) |
| 지원 기기 | 20,216대 |
| 서명 | Play 앱 서명 + 새 업로드 키 (23-6-3). **키 관련 경고 없이 정상 접수** |

- 첫 업로드가 업로드 키를 확정하는 시점이었는데 아무 경고 없이 통과했다.
  23-6-3 의 판단(번들 0건이면 업로드 키 미고정)이 옳았음이 확인됐다.
- AI 애셋 선언: **그래픽 이미지만** AI 생성으로 라벨 지정. 스크린샷 7장(오픈소스 목업
  템플릿에 직접 스타일링)과 앱 아이콘(기존 일러스트)은 라벨 없음.

### 24-3. 무시해도 되는 경고
`미리보기 및 확인` 에서 경고 1개가 뜬다:
> 이 App Bundle 유형과 연결된 가독화 파일이 없습니다.

`build.gradle` 이 `minifyEnabled false` 라 **애초에 난독화를 하지 않으므로** 올릴 매핑
파일이 없는 게 정상이다. 크래시 로그도 그대로 읽힌다. 정보성 안내이며 제출을 막지 않는다.

### 24-4. Play Console UI 함정 (다음에 또 헤맬 만한 것들)
- **`새 버전 만들기` 버튼이 회색** = 새로 만들 수 없다는 뜻이 아니라, **이미 임시 버전이
  있어서** 또 못 만드는 것. `출시` 탭의 `버전 수정` 으로 이어서 작성한다
- **출시 노트의 `<ko-KR>` 태그는 지우면 안 된다.** 안쪽 안내 문구만 지우고 그 자리에 쓴다.
  제대로 들어가면 `0개의 언어` → `1개의 언어로 출시 노트 제공됨` 으로 바뀐다
- **저장 ≠ 제출.** 저장은 `게시 개요` 에 쌓아둘 뿐이고, 거기서
  **`검토를 위해 변경사항 N개 제출`** 을 눌러야 심사가 시작된다
- **그래픽 이미지는 `스토어 등록정보 → 1단계 애셋`** 에 있다. 2단계 `리뷰` 는 읽기 전용이라
  업로드 칸이 없다. 자세한 설명 아래로 한참 스크롤해야 나온다
- `스토어 등록정보`(이미지·설명) 와 `스토어 설정`(카테고리·태그·연락처) 은 다른 메뉴다
- **`관리형 게시`가 꺼져 있으면 승인 즉시 자동 공개**된다. 공개 시점을 고르려면 켜둘 것

### 24-5. ✅ 안드로이드 음성 암송 실기기 확인 완료 (2026-08-30, 사용자 직접 테스트)
23-6-5 에서 AAB 내부에 `SpeechRecognition` 클래스와 `RECORD_AUDIO` 권한이 들어간 것까지만
확인하고 실기기 검증 없이 프로덕션에 제출했던 부분. 사용자가 직접 실기기에서 확인 —
마이크 권한 팝업, 음성 인식, 텍스트 변환까지 **정상 동작**. `versionCode 2` 긴급 대응 불필요.

### 24-6. 다음 작업
- [ ] 심사 결과 대기 — 신규 앱 + 조직 계정 첫 출시라 최대 7일. 결과는 contact@testbench.tools 로
- [ ] 공개 즉시 **음성 암송 실기기 확인** (24-5)
- [ ] 위젯 배치, 알림, 이번 주 말씀 표시 확인
- [ ] `새키_비밀번호.txt` 클라우드·외장 백업 (아직 이 PC 한 곳뿐)
- [ ] 옛 키(`church-memory-key`) 와 `keystore-backup-2026-08-29.zip` 정리
- [ ] 반려 시 사유 확인 후 대응 (iOS 는 2.1 정보요청을 실기기 영상으로 통과시켰다 — 21장)
- [ ] 출시 확정되면 README 의 Google Play 배지를 `출시 준비 중` → 스토어 링크로 교체

---

**문서 갱신**: 2026-08-29
**작성**: Claude Code 세션 (Play 프로덕션 제출 라운드)

---

## 25. Apple 개인→조직 전환 — 1차 좌초 (2026-08-27 ~ 09-02)

> ⚠️ **후속**: 이 장의 결론("취소, 개인 계정 유지")은 뒤집혔다. 취소 회신 직후
> Apple 이 수동 검증으로 전환을 승인했다. **최종 결과는 26장 참조.**

Play Console 조직 전환(23장)에 성공한 뒤, 같은 DUNS 로 Apple Developer 계정도 조직으로
바꾸려 했다. **결론부터: 실패했고 취소했다. Apple 은 개인 계정으로 유지한다.**

### 25-1. 결정적 사실 — Apple 은 법인만 받는다

`TECHSENSE` 는 **법인이 아니라 개인사업자**다. Apple 은 조직 계정에 **법인격**을 요구한다.

조직 정보 입력 화면에서 DUNS 조회 결과:
```
이 조직이 법인임을 확인할 수 없습니다.
자영업자/1인 기업의 경우 개인으로 등록해 주십시오.
해당 조직이 법인으로 기재되어야 하는 경우 D&B 프로필을 업데이트해 주십시오.
```

등록 첫 화면 안내에도 명시되어 있었다:
> "조직이 **법인인 경우에만** Apple 과 법적 계약을 체결할 수 있습니다.
> DBA, 허위 사업자, 상표명 또는 지사는 허용되지 않습니다."
> "법인으로 인정되지 않은 **개인사업자 및 1인 기업**은 … **등록자 개인의 법적 이름 하에** 기재됩니다."

즉 개인사업자는 억지로 진행해도 판매자명이 결국 개인 실명이 되므로 **실익이 없다.**

| 스토어 | 개인사업자의 조직 계정 | 결과 |
|---|---|---|
| Google Play | 인정함 | ✅ 전환 성공 (23장) |
| Apple | **법인만 허용** | ❌ 차단 |
...
| 날짜 | 내용 |
|---|---|
| 08-27 | DUNS 발급 (696602847 / TECHSENSE) — 신청 4일 만 |
| 08-29 | `testbench.tools` footer 에 "operated by TECHSENSE" 추가, `contact@testbench.tools` 포워딩 설정.<br>Apple "Individual to Organization Membership Update" 폼 제출 |
| 09-01 | 개발자 지원(Neil) 안내 메일 4개 항목 확인 후 "진행해 달라" 회신.<br>**계정 이메일이 `fnsf92@naver.com`** 이라 gmail 만 보다가 며칠 늦게 발견 |
| 09-02 | 조직 등록 폼에서 DUNS 법인 검증 실패 → **취소 회신** |

### 25-3. 헛수고가 아니었던 것

- **DUNS 번호** — Play Console 조직 계정 인증에 실제로 사용됨. 이게 없었으면 Play 전환도 불가했다
- **Play 조직 전환** — 12명/14일 비공개 테스트 게이트 면제 → 프로덕션 직행 (24-1)
- **testbench.tools 정비** — 회사 정보로 계속 유효

**순수 손실**: Apple 신청서 작성 + 메일 왕복 + 전환 기간 동안 **인증서·식별자·프로파일 포털 잠김**
(App Store Connect 는 정상 동작했고, 급한 iOS 업데이트가 없어 실피해는 없었다).

### 25-4. 교훈

> **스토어 조직 계정 이야기가 나오면 가장 먼저 사업자 형태(법인 / 개인사업자)를 확인할 것.**

이번엔 순서를 틀렸다. "회사 웹사이트·이메일이 없다" 는 걸 먼저 붙잡고 그것부터 해결하느라
정작 **가장 앞에서 걸러졌어야 할 법인 여부**를 뒤늦게 확인했다.

- **DUNS 보유 ≠ 법인.** 개인사업자도 DUNS 를 발급받을 수 있다. DUNS 가 있다는 사실은
  법인 여부에 대해 아무것도 보장하지 않는다
- 스토어마다 "조직" 의 정의가 다르다. Play 에서 됐다고 Apple 에서 되는 게 아니다

### 25-5. 결말

취소 회신을 보냈으나 **Apple 이 그 사이 전환을 승인**했다. 26장으로 이어진다.

---

**문서 갱신**: 2026-09-02 (2026-09-04 후속 주석 추가)
**작성**: Claude Code 세션 (Apple 조직 전환 1차 좌초 정리)

---

## 26. Apple 조직 계정 전환 완료 + 유료 앱 준비 (2026-09-03 ~ 09-04)

25장에서 "법인이 아니라 불가"로 판단해 취소 회신까지 보냈으나, **Apple 이 수동 검증으로
전환을 승인**했다. 결과적으로 원하던 조직 계정을 얻었다.

### 26-1. 뒤집힌 경위

| 시각 | 내용 |
|---|---|
| 09-02 | 조직 등록 폼에서 DUNS 법인 검증 실패 → 취소하기로 결정 |
| 09-03 11:43 | Apple: **"D-U-N-S 정보를 확인했습니다"** — 재검증 통과 |
| 09-03 20:16 | 조직 등록 접수 (등록 ID `7M6UMRTK8H`) |
| 09-04 09:55 | 계약 서명 → **멤버십이 TECHSENSE 에 할당 완료** |
| 09-04 10:05 | 취소 요청 지원 케이스(`102953247027`)가 **뒤늦게** 접수 — 승인이 먼저였다 |

지원팀(Neil)이 수동으로 통과시킨 것으로 보인다. **Team ID `HFDSR2HFT4` 는 그대로**이고
기존 앱·인증서도 전부 승계됐다. iTunes Lookup API 로 확인한 결과 스토어 판매자명이
`Hakseong Kim` → **`TECHSENSE`** 로 즉시 반영됐다.

> 취소 케이스가 살아 있으면 승인된 전환이 되돌려질 수 있으므로 **철회 회신**이 필요했다.

### 26-2. 개인사업자 우려의 해소 — Apple 한국 흐름은 BRN 을 받는다

25장에서 "개인사업자는 Apple 조직 계정 불가"라고 결론냈는데, 실제로는 **한국 개발자용
흐름이 사업자등록번호(BRN)를 받도록 설계**되어 있었다.

`비즈니스 → 대한민국 비즈니스 연락처 정보` 화면의 문항:
```
본인은 유효한 사업자등록번호(BRN)를 가진 대한민국에 거주하는 개발자입니다.  → 예
```
법인등록번호가 아니라 **BRN** 을 묻는다. 개인사업자도 사실대로 `예` 로 답하면 된다.

- 대한민국 세금 양식도 **90일 이내 발행 사업자등록증 업로드**가 전부였다(영문본 업로드)
- 즉 Apple 의 정책 문구는 "법인만"이지만, 한국 개인사업자를 수용하는 실무 경로가 존재한다

### 26-3. 비즈니스 3개 작업 처리

전환 직후 `비즈니스 → TECHSENSE` 에 3개 작업이 떴다.

| 작업 | 처리 |
|---|---|
| EU 디지털 서비스법(DSA) 거래자 자격 | 완료 → `활성화됨` |
| 대한민국 전자상거래법 연락처 정보 | 완료 → `활성화됨` |
| 유료 앱 계약 전 법인 정보 업데이트 | BRN 제출로 자동 충족(배너 소멸) |

기존 `Hakseong Kim` 실체는 **`사용 중지됨`(계약 없음)** 으로 바뀌고, 무료 앱 계약
(2026-06-11~2027-06-12)이 TECHSENSE 로 정상 승계됐다.

> ⚠️ **한국 전자상거래법 연락처는 App Store 제품 페이지에 공개 표시된다.**
> 전화번호·이메일·주소가 노출되므로, 23-7 에서 자택 겸용이라 방침 본문에 주소를 뺐던
> 판단과는 별개로 **여기서는 법적 의무라 피할 수 없다.**

### 26-4. 유료 앱 준비 — 은행 계좌

`유료 앱 계약` 을 서명하면 **은행 계좌 + 세금 양식 2종**을 요구한다.

- 계좌: 기업은행 **개인 명의** 계좌 사용. **한국 은행 코드 `003`**
- 식별 번호 유형: **주민등록번호** — 계좌가 개인 명의이므로 이게 맞다.
  사업자등록번호를 넣으면 은행 실명확인 대조가 실패한다
- 예금주 주소 검증에서 *"제출한 주소를 확인할 수 없습니다"* 경고 → **`확인` 눌러 진행 가능**
  (한국 도로명주소가 해외 검증 서비스에서 자주 실패한다. 하드 블록 아님)
- 등록 후 **24시간 처리 대기**, 그동안 추가 변경 불가

> 향후 매출이 커지면 **사업자 통장으로 교체 권장**. Apple 은 예금주명이 법적 실체명과
> 일치하기를 원하고, 세무상 매출 구분도 깔끔해진다.

### 26-5. 미국 세금 양식 — 개인사업자의 정답

두 가지가 뜬다. 이름이 헷갈리는데 **W-8BEN(개인용)의 정식 명칭이 "Certificate of Foreign
Status of Beneficial Owner"** 다.

| 양식 | 성격 |
|---|---|
| U.S. Certificate of Foreign Status of Beneficial Owner | Apple 자체 간이 증명서 (미국 내 직원·자산 없음 선언) |
| U.S. Substitute Form W-8BEN-E | IRS 양식 (조세조약 혜택 신청) |

**둘 다 제출해야 한다.**

**핵심: Type of Beneficial Owner / Chapter 3 Status → `Disregarded entity`**

드롭다운에 **`Individual` 옵션이 없다**(법인 유형 전용 목록). 개인사업자는
"소유자와 분리되지 않아 세법상 별개 실체로 보지 않는 사업체" = **disregarded entity** 가
정확한 미국 세법 개념이다. `Corporation` 을 고르면 사실과 다른 신고가 된다.
**두 양식에서 동일하게** 선택할 것.

- Title: `Owner`
- 하이브리드 조약 청구 여부: **`No`** (개인사업자는 양국 모두 개인 소득으로 봄)

**Part III 조세조약**
- 14a·14b 체크
- LOB: **`No LOB article in treaty`** ← 한미 조세조약은 **1976년 체결·1979년 발효**된
  구 조약이라 포괄적 LOB 조항이 없다. IRS 도 "LOB 조항 없는 국가 거주자는 추가 요건 불요"
  라고 명시한다
- 14c(배당·이자): 체크 안 함
- **9b Foreign TIN 에 사업자등록번호 필수** — Tips Sheet 에 *"조약 감면 세율을 받으려면
  U.S. TIN 또는 Foreign TIN 이 필요하다"* 고 명시. 8번 U.S. TIN 은 비움

**⚠️ 15번(Special rate and Conditions)은 비운다.**
처음에 "Article 14, paragraph 2, 10%" 를 채우라고 안내했으나 **틀렸다.**
Apple `Substitute Form W-8BEN-E Tips Sheet` 원문:
> *"For item Line 15: Complete this item **only if** you are eligible to claim treaty benefits
> that requires you to meet conditions **not covered by** lines 14a through 14c.
> **It is expected that Line 15 would not normally be applicable.**"*

일반적인 로열티 조약 세율은 **14a·14b 체크만으로 적용**된다. 15번은 면세단체 등 특수
케이스용이며, `Income from the sale of applications` 체크박스도 15번 소속이므로 함께 비운다.

> **교훈: IRS 일반론보다 Apple 이 제공하는 Tips Sheet 를 먼저 볼 것.** 양식 상단
> 다운로드 링크에 있다. 제출 후 수정 불가 + 위증 서약이 붙는 서류다.

### 26-6. 현재 상태 (2026-09-04 기준)

| 항목 | 상태 |
|---|---|
| 무료 앱 계약 | 활성화됨 |
| 유료 앱 계약 | 처리 중 |
| 은행 계좌 (기업은행 1014) | 처리 중 (24시간) |
| U.S. Certificate of Foreign Status | **활성화됨** |
| U.S. Substitute Form W-8BEN-E | **활성화됨** |
| 대한민국 세금 양식 | 대기 중 (사업자등록증 검토) |
| 디지털 서비스법 / 대한민국 전자상거래법 | 활성화됨 |

전부 활성화되면 유료 앱·인앱결제 배포가 가능해진다.

### 26-7. 남은 작업

- [ ] 취소 케이스 `102953247027` 철회 회신 (승인된 전환이 되돌려지지 않도록)
- [ ] 은행 계좌·대한민국 세금 양식 승인 확인 → 유료 앱 계약 `활성화됨` 전환 확인
- [ ] Play Store 프로덕션 심사 결과 (8/29 제출, 최대 7일 → 9/9 경까지 정상 범위)
- [ ] 새 업로드 키 비밀번호 클라우드·외장 백업 (아직 윈도우 PC 한 곳)
- [ ] 심사용 영상 릴리즈(`review-demo-v1`) 정리
- [ ] Play 출시 확정 시 README 배지를 스토어 링크로 교체

---

**문서 갱신**: 2026-09-04
**작성**: Claude Code 세션 (Apple 조직 전환 완료 + 유료 앱 계약·세금 양식 라운드)
