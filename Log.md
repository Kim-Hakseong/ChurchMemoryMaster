...
### 24-5. ⚠️ 검증되지 않은 채 나간 것
**안드로이드 음성 암송이 실기기에서 확인된 적이 없다.**

23-6-5 에서 AAB 내부에 `SpeechRecognition` 클래스와 `RECORD_AUDIO` 권한이 들어간 것까지는
확인했으나, 실제 마이크 동작은 미검증이다. 오늘 처음 고친 부분이라(`7989a19` — 플러그인이
`package.json` 에만 있고 안드로이드 capacitor 생성 파일에 반영된 적이 없었다) 위험이 남아 있다.

- 내부 테스트를 건너뛰고 프로덕션 직행을 선택했으므로, 문제가 있으면 사용자가 먼저 만난다
- **공개되면 가장 먼저 확인할 것.** 문제 시 `versionCode 2` 로 즉시 대응한다
