# 플레이스토어 출시 — 최종 등록 문구 (2026-08-29 확정)

> 아래는 **그대로 복사해 붙여넣는** 최종본입니다.
> 이 문서 뒤쪽의 단계별 가이드는 계획 단계에 쓴 것으로, 키스토어 정보 등 일부가 낡았습니다.
> 실제 진행 상황은 `Log.md` 23장, 빌드 절차는 `HANDOFF-WINDOWS.md` 를 보세요.
...
```
유치부·초등부·중고등부 주간 암송 말씀. 빈칸 채우기와 음성 암송으로 매일 꾸준히.
```

## 자세한 설명 (1157자 / 4000자 제한)
...
| 항목 | 규격 | 파일 |
|------|------|------|
| 앱 아이콘 | 512×512 PNG | `client/public/icon-512.png` (그대로 사용) |
| 그래픽 이미지 | 1024×500 PNG | 이 세션에서 생성 — 앱 아이콘 + 티얼 그라데이션(`#0AAFAF`) |
| 휴대전화 스크린샷 | 2~8장, 16:9 또는 9:16 | **미확보** |

> 그래픽 이미지는 `sharp` 로 SVG 를 렌더링해 만들었다. 한글은 `font-family="Malgun Gothic"` 로
> 지정해야 렌더링된다. 텍스트가 오른쪽 끝에 닿으면 일부 화면에서 잘리므로 여백을 둘 것.

## 스토어 설정

- 앱 또는 게임: **앱**
- 카테고리: **교육**
- 태그: `교육` 유지. **`자기 계발` 은 성인 자기계발 그룹이라 부적합** — 종교/영성, 어린이,
  학습 도구 계열로 교체 권장 (Google 권장: 억지로 5개 채우지 말고 명백히 관련된 2~3개)
- 연락처: `contact@testbench.tools` / `+821034368776` / `https://testbench.tools/`
- 외부 마케팅: 켜둠. 가족 정책상 아동 대상 앱의 외부 마케팅을 막는 조항은 없음
  (리마케팅 금지 조항은 *앱 내 광고* 에 관한 것이며 이 앱은 광고가 없음).
  끄면 반영에 60일 소요
...
| 항목 | 답변 | 근거 |
|------|------|------|
| 로그인 세부정보 | 제한된 부분 없음 | 로그인·결제 없음 |
| 광고 | 없음 | AAB 검사: AdMob/Firebase Analytics/Facebook/AppsFlyer 전부 없음 |
| 광고 ID | 사용 안 함 | AAB 검사: `AD_ID` 권한 없음, `AdvertisingIdClient` 없음 |
| 콘텐츠 등급 | 전체이용가 / 3세 이상 | 카테고리 `다른 모든 앱 유형`, 설문 전 항목 아니요 |
| 타겟층 | 아동 포함 → 가족 정책 준수 | 유치부·초등부 화면이 실재 |
| 데이터 보안 | 수집·공유 없음 | 전부 로컬 저장, 개발자 서버 없음 |
| 건강 앱 | 해당 없음 | AAB 검사: Health Connect/Google Fit/BODY_SENSORS 없음 |
| 개인정보처리방침 | https://kim-hakseong.github.io/ChurchMemoryMaster/privacy-policy.html | GitHub Pages HTTP 200 확인 |

---

# (참고) 계획 단계 가이드 — 일부 내용이 낡았음

> ⚠️ 2단계의 키스토어 파일명·별칭은 실제와 다릅니다.
> 실제 키: `C:/Users/user/keystores/church-memory-2026.jks`, 별칭 `key0` (Log.md 23-6-3)
